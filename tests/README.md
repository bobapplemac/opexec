...
- `OpExec.SshAgent.Tests/OpenSshTestKey.cs` contains an unencrypted Ed25519 OpenSSH
  private key and its public key, with comment `opexec-milestone-6-test-key`.
- `OpExec.OnePassword.Tests/OpenSshTestKey.cs` contains the same private key for
  fake 1Password responses.
- `OpExec.SshAgent.Tests/TestIdentityProvider.cs` contains a deterministic public
...
Run `dotnet test OpExec.slnx -c Release` on both Windows and Linux. The suite uses
